...
resolve_rtc_ip() {
  ...
  if [[ -z "$ip" ]]; then
    echo "error: could not resolve RTC IP for host $host" >&2
    exit 1
  fi
  printf '%s\n' "$ip"
}
...
detect_host() {
  ...
  if [[ -n "${LIVEKIT_URL:-}" ]]; then
    local extracted
    extracted="$(printf '%s' "$LIVEKIT_URL" | sed -E 's#^[a-zA-Z]+://([^/:]+).*#\1#')"
    if [[ -n "$extracted" ]]; then
      printf '%s\n' "$extracted"
      return
    fi
...
replace_all "$OUT/livekit.yaml" "__QCHAT_PUBLIC_HOST__" "$HOST"
replace_all "$OUT/livekit.yaml" "__QCHAT_RTC_IP__" "$RTC_IP"
...
replace_all "$OUT/coturn.conf" "__QCHAT_RTC_IP__" "$RTC_IP"
...
QCHAT_PUBLIC_HOST=${HOST}
LIVEKIT_NODE_IP=${RTC_IP}
...
if [[ -f "$API_ENV" ]]; then
  tmp="$(mktemp)"
  grep -vE '^(QCHAT_PUBLIC_HOST|LIVEKIT_NODE_IP|LIVEKIT_URL|LIVEKIT_API_KEY|LIVEKIT_API_SECRET)=' "$API_ENV" >"$tmp" || true
  {
    echo "QCHAT_PUBLIC_HOST=${HOST}"
    echo "LIVEKIT_NODE_IP=${RTC_IP}"
...
cat <<EOF
Rendered media config for host: ${HOST} (RTC IP ${RTC_IP})
  ${OUT}/livekit.yaml
  ${OUT}/coturn.conf
  ${OUT}/media.env
  ${WEB_ENV}
