...
echo "== phone change challenge =="
PHONE="137$(date +%s | tail -c 9)"
CH=$(curl -sS -X POST "$API/v1/me/phone/request" -H "Authorization: Bearer $AT" -H 'Content-Type: application/json' -d "{\"new_phone\":\"$PHONE\"}")
echo "$CH" | python3 -c 'import sys,json; d=json.load(sys.stdin); assert d.get("challenge_id"), d; print("phone challenge ok", "dev_code" in d)'
