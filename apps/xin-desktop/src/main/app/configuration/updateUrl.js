...
function readJsonUpdateUrl(filePath) {
  try {
    ...
    return String(raw?.updateUrl || raw?.XINCHAT_UPDATE_URL || raw?.QCHAT_UPDATE_URL || "")
      .trim()
      .replace(/\/$/, "");
  } catch {
    return "";
 ...
 * Precedence: XINCHAT_UPDATE_URL → QCHAT_UPDATE_URL → userData/config.json → production.json
 */
function resolveUpdateUrl() {
  ...
  const fromEnv = String(
    process.env.XINCHAT_UPDATE_URL || process.env.QCHAT_UPDATE_URL || ""
  )
    .trim()
    .replace(/\/$/, "");
