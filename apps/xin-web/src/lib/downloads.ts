...
import { withBasePath } from "./brand";
...
export function downloadHref(app: DownloadApp): string | null {
  ...
  if (app.file) return withBasePath(`/downloads/${encodeURIComponent(app.file)}`);
  return null;
}
...
export async function loadDownloadManifest(): Promise<DownloadManifest> {
  const res = await fetch(withBasePath(`/downloads/manifest.json?t=${Date.now()}`), {
    cache: "no-store",
  });
