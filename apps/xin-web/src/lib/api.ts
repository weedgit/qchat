...
export function formatSendError(err: unknown, fallback = "Failed to send"): string {
  if (err instanceof ApiError) {
    if (err.status === 413) return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.fileTooLarge");
    if (err.status === 401) return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.unauthorized");
    if (err.status === 0) return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.network");
    if (err.status === 400 && /not allowed|file/i.test(err.message)) {
      return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.fileNotAllowed");
    }
    if (err.status !== undefined && err.status >= 500) {
      return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.server");
    }
    const key = apiErrorMessageKey(err, "api.err.sendFailed");
    if (key !== "common.error") return formatApiErrorLocale(err, getStoredResolvedLocale(), key);
  }
  if (fallback === "Upload failed") return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.uploadFailed");
  return formatApiErrorLocale(err, getStoredResolvedLocale(), "api.err.sendFailed");
}
